...
public final class ProfessorPushCore {
    ...
    public static final String STATUS_CHANNEL_ID = "professor_service_hidden_v2";
    public static final String LEGACY_STATUS_CHANNEL_ID = "professor_service_status";
    ...
    public static void ensureChannels(Context ctx) {
        try {
            ...
            // قناة الحالة مخفية كلياً (أهمية صفر): الخدمة الأمامية تعمل بكامل قوتها
            // والإشعار الإجباري الخاص بها لا يظهر في لوحة الإشعارات إطلاقاً.
            if (manager.getNotificationChannel(STATUS_CHANNEL_ID) == null) {
                NotificationChannel status = new NotificationChannel(STATUS_CHANNEL_ID, "خدمة التنبيهات (مخفية)", NotificationManager.IMPORTANCE_NONE);
                status.setDescription("قناة نظام مخفية بالكامل — تُبقي خدمة التنبيهات الفورية تعمل دون أي إشعار ظاهر");
                status.setShowBadge(false);
                status.enableVibration(false);
                status.setSound(null, null);
                manager.createNotificationChannel(status);
            }
            // حذف قناة الحالة القديمة الظاهرة من الأجهزة المحدّثة حتى لا يتبقى لها أثر
            try { manager.deleteNotificationChannel(LEGACY_STATUS_CHANNEL_ID); } catch (Exception ignored) {}
    ...
    /** إشعار الخدمة الأمامية الإجباري — قناته مخفية كلياً فلا يظهر للمستخدم أبداً. */
